...
public class Hovl_Laser2 : MonoBehaviour
{
    ...
    void Update()
    {
        if (laserPS != null && !UpdateSaver)
        {
            ...
            // Define a layer mask to ignore specific layers
            int layerMask = ~(1 << LayerMask.NameToLayer("Muro"));  // Ignora objetos en la capa "IgnorarLáser"

            RaycastHit hit;
            // Lanzar el raycast con el layerMask
            if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, MaxLength, layerMask))
            {
                // Si golpea un objeto válido, el láser se comportará como antes
            ...
            {
                // Si no golpea ningún objeto, se extiende a la distancia máxima
